...
defmodule Storybook.CarbonComponents.RadioButton.Interactivity do
  ...
  def doc do
    "Radio buttons interactivity."
  end

  @impl true
  def mount(_params, _session, socket) do
    {:ok, assign(socket, last_event: nil, value: "a", checked: nil)}
  end
  ...
  def render(assigns) do
    assigns =
      assign(
        assigns,
        :event_attrs,
        custom_event_attrs(["cds-radio-button-group-changed", "cds-radio-button-changed"])
      )

    ~H"""
    <.radio_button_group
      id="radio-button-events"
      legend_text="Choose one"
      name="choices"
      value={@value}
      {@event_attrs}
    >
