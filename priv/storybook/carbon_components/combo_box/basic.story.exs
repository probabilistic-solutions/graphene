defmodule Storybook.CarbonComponents.ComboBox.Basic do
  ...
  def variations do
    [
      %Variation{
        id: :default,
        attributes: %{title_text: "Choose an option", label: "Combo box"},
        slots: [
...
<:item value="edge">Edge</:item>|
      ...
      %Variation{
        id: :inline,
        attributes: %{title_text: "Inline combo", label: "Inline", type: "inline"},
        slots: [
        ...
        variations:
          for size <- ~w(sm md lg) do
            %Variation{
              ...
              attributes: %{title_text: "Size #{size}", label: "#{String.upcase(size)}", size: size},
              slots: [
