...
(function(SELECTION_FEEDBACK_OPTION2) {
  SELECTION_FEEDBACK_OPTION2["FIXED"] = "fixed";
  SELECTION_FEEDBACK_OPTION2["TOP"] = "top";
  SELECTION_FEEDBACK_OPTION2["TOP_AFTER_REOPEN"] = "top-after-reopen";
})(SELECTION_FEEDBACK_OPTION || (SELECTION_FEEDBACK_OPTION = {}));

// node_modules/@carbon/web-components/es/components/multi-select/multi-select.scss.js
...
// node_modules/@carbon/web-components/es/components/multi-select/multi-select.js
var CDSMultiSelect = class CDSMultiSelect2 extends CDSDropdown$1 {
  constructor() {
    super(...arguments);
  ...
  _selectionDidChange(itemToSelect) {
    ...
    if (itemToSelect) {
      if (itemToSelect.isSelectAll) {
        const items = this.filterable ? Array.from(this.querySelectorAll(this.constructor.selectorItemResults)) : allItems;
        items.forEach((i2) => {
          if (!i2.isSelectAll && !i2.disabled) {
            i2.selected = !itemToSelect.selected;
          }
          i2.indeterminate = false;
        });
  ...
  _handleClickInner(event) {
    ...
    if (((_a = this._selectionButtonNode) === null || _a === void 0 ? void 0 : _a.contains(event.target)) && !this.readOnly) {
      ...
      this._handleUserInitiatedClearInput();
    } else if (!((_c = event.target) === null || _c === void 0 ? void 0 : _c.matches(this.constructor.aiLabelItem)) && // remove reference to slug in v12
  ...
  _handleKeypressInner(event) {
    ...
    if (((_a = this._clearButtonNode) === null || _a === void 0 ? void 0 : _a.contains(event.target)) && // Space key should be handled by `<input>` unless "clear selection" button has focus
  ...
  _handleMouseoverInner(event) {
    ...
    if (!item || isFiltering || !item.hasAttribute("selected") || !item.hasAttribute("highlighted")) {
      return;
    }
  ...
  _renderPrecedingLabel() {
    ...
    const selectionButtonClasses = e2({
    ...
    return selectedItemsCount === 0 ? void 0 : b`
          <div
            id="selection-button"
            role="button"
  ...
  _renderFollowingLabel() {
    ...
    return filterInputNode && filterInputNode.value.length > 0 && this.filterable ? b`
          <div
            id="clear-button"
            role="button"
            ...
            tabindex="0"
            title="${clearSelectionLabel}">
            ${iconLoader(_16, { "aria-label": clearSelectionLabel })}
          </div>
        ` : void 0;
  }
  /**
  ...
  _handleInput() {
    ...
    if (this.selectAll) {
      ...
      if (selectAllItem) {
        ...
        const actionable = visible.filter((i2) => !i2.isSelectAll && !i2.disabled);
    ...
    if (visibleItems.length > 0) {
      visibleItems.forEach((i2) => i2.removeAttribute("highlighted"));
      this.setAttribute("item-clicked", "");
  ...
  updated(changedProperties) {
    ...
    if (this.selectAll && changedProperties.has("open") && this.open) {
      ...
      const selectAllItem = items.find((i2) => i2.isSelectAll);
      if (selectAllItem) {
        this.appendChild(selectAllItem);
        items.filter((i2) => i2 !== selectAllItem).forEach((i2) => this.appendChild(i2));
      }
    }
  ...
  _computeSelectAllState() {
    if (!this.selectAll)
      return;
    const allItems = Array.from(this.querySelectorAll(this.constructor.selectorItem));
    const selectAllItem = allItems.find((i2) => i2.isSelectAll);
    if (!selectAllItem || selectAllItem.hasAttribute("filtered")) {
      return;
    }
    const enabledItems = allItems.filter((i2) => !i2.isSelectAll && !i2.disabled).filter((i2) => !this.filterable || !i2.hasAttribute("filtered"));
    const selectedCount = enabledItems.filter((i2) => i2.selected).length;
